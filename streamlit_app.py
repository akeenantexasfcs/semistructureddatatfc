...
def create_excel_from_json(json_data):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # Header
        headers = ["Name/Term", "LGD", "% RR Used", "% AGG Used", "Used", "Available", "Total Exposure", "% TE of RR", "% TE of AGG"]
        ...
        for entry in json_data["entries"]:
            if "name" in entry:
                ...
                for col_num in range(1, 10):
                    worksheet.cell(row=current_row, column=col_num).fill = PatternFill(start_color='FFFFCC', end_color='FFFFCC', fill_type='solid')
                current_row += 1
